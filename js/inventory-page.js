...
function renderInventory(items) {
  ...
  for (const item of items) {
    ...
    card.className = "inv-card" + (isShow ? " on-showcase" : "") + (isListed ? " on-market" : "");
    ...
    card.innerHTML = `
      ...
      <div class="inv-card-actions">
        ${!isListed && !isShow ? `<button class="btn btn-ghost btn-xs js-showcase" title="${isShow ? "Снять с витрины" : "На витрину"}">
          ${isShow ? "📤 Снять" : "🖼 Витрина"}
        </button>` : ""}
        ${isShow ? `<button class="btn btn-ghost btn-xs js-unshowcase">📤 Снять</button>` : ""}
        ${!isListed && !isShow ? `<button class="btn btn-ghost btn-xs js-market">🏪 Рынок</button>` : ""}
        ${isListed ? `<button class="btn btn-ghost btn-xs js-unlist">❌ Снять</button>` : ""}
        ${!isListed && !isShow ? `<button class="btn btn-ghost btn-xs js-sell" title="Продать за ${sellPrice} PS">
          💰 ${sellPrice} PS
        </button>` : ""}
    ...
    // На рынок
    card.querySelector(".js-market")?.addEventListener("click", () => {
      openListModal(item);
    });
    ...
    <div class="modal" style="max-width:420px">
      ...
      <div class="modal-body">
        ...
        <div style="display:flex;align-items:center;gap:16px;margin:20px 0">
          <img src="${esc(item.image)}" alt="" style="width:64px;height:64px;border-radius:8px;object-fit:cover">
          <div>
            <h3>${esc(item.name)}</h3>
...
function toggleContractItem(invId, rarity, card) {
  ...
  if (idx !== -1) {
    ...
    // Проверяем — все должны быть одной редкости
...
function updateContractPanel() {
  ...
  if (selectedContract.length > 0) {
    ...
    if (item) {
      const upgraded = nextRarity(item.rarity);
